...
describe("MessageTemplatesService", () => {
  ...
  describe("listChildren", () => {
    it("returns children from GET /message_templates/{id}/message_templates", async () => {
      const childTemplate = { ...sampleTemplate, id: "rsp_child_1", name: "Child Template" };
      server.use(
        http.get(`${BASE}/message_templates/rsp_123/message_templates`, () =>
          HttpResponse.json({ _results: [childTemplate], _pagination: {} }),
        ),
      );

      const result = await service.listChildren({ action: "list_children", template_id: "rsp_123" });
      expect(result.results).toHaveLength(1);
      expect(result.results[0]?.id).toBe("rsp_child_1");
    });

    it("passes page_token and limit as query params", async () => {
      let capturedUrl = "";
      server.use(
        http.get(`${BASE}/message_templates/rsp_123/message_templates`, ({ request }) => {
          capturedUrl = request.url;
          return HttpResponse.json({ _results: [], _pagination: {} });
        }),
      );

      await service.listChildren({ action: "list_children", template_id: "rsp_123", page_token: "childpage", limit: 10 });
  ...
  describe("createChild", () => {
    it("sends POST /message_templates/{id}/message_templates with name and body", async () => {
      let capturedBody: unknown;
      const childTemplate = { ...sampleTemplate, id: "rsp_child_2", name: "Child Template" };
      server.use(
        http.post(`${BASE}/message_templates/rsp_123/message_templates`, async ({ request }) => {
          capturedBody = await request.json();
          return HttpResponse.json(childTemplate, { status: 201 });
        }),
      );
      ...
      expect(capturedBody).toMatchObject({ name: "Child Template", body: "<p>Child body</p>" });
      expect(result.id).toBe("rsp_child_2");
    });

    it("includes optional subject when provided", async () => {
      let capturedBody: unknown;
      server.use(
        http.post(`${BASE}/message_templates/rsp_123/message_templates`, async ({ request }) => {
          capturedBody = await request.json();
          return HttpResponse.json({ ...sampleTemplate, id: "rsp_child_3" }, { status: 201 });
        }),
      );
      ...
      expect(capturedBody).toMatchObject({ subject: "Child Subject" });
    });
  });
});
