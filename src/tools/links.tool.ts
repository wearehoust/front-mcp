...
export function registerLinksTool(
  ...
  registerTool(
    ...
    DESCRIPTION,
    {
      action: z.enum(["list", "get", "create", "update", "list_conversations"]).describe("The action to perform"),
    ...
    async (params) => {
      try {
        ...
        if (action === "list" || action === "list_conversations") {
          ...
          return formatSuccess(sanitized, summarizeList("links", count, resultObj.next_page_token));
        }
        if (action === "get") {
          return formatSuccess(sanitized, summarizeGet("link", (params["link_id"] as string | undefined) ?? "unknown"));
        }
        return formatSuccess(sanitized, summarizeAction("link", action, params["link_id"] as string | undefined));
      } catch (error: unknown) {
