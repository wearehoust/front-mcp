  ...
  registerTool(
    ...
    async (params) => {
      try {
        ...
        if (action === "get") {
          return formatSuccess(sanitized, summarizeGet("message_template", (params["template_id"] as string | undefined) ?? "unknown"));
        }
        return formatSuccess(sanitized, summarizeAction("message_template", action, params["template_id"] as string | undefined));
      } catch (error: unknown) {
